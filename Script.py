...
# -------------------- Model config --------------------
MODEL_PATH = "c:/Users/Rishi/Desktop/project/models/Mobilenetv2_ISL_model.tflite"
...
# Safe model loader: supports Keras (.h5/.keras/.hdf5), SavedModel dir and a basic TFLite wrapper
model = None
...
class TFLiteModelWrapper:
    ...
    def predict(self, x_batch, verbose=0):
        # x_batch expected shape: (1, H, W, C)
        inp = x_batch.astype(np.float32)
...
def try_load_model(path):
    ...
    try:
        if os.path.isdir(path):
            ...
            print(f"Trying to load SavedModel from directory: {path}")
            return tf.keras.models.load_model(path)
        ext = pathlib.Path(path).suffix.lower()
        if ext in ('.h5', '.keras', '.hdf5'):
            try:
                print(f"Trying to load Keras model file: {path}")
                return tf.keras.models.load_model(path)
            except OSError as e:
                # Typical "file signature not found" or corrupted h5
                print(f"HDF5 load error for '{path}': {e}")
                return None
            except Exception as e:
                print(f"Keras load error for '{path}': {e}")
                return None
        if ext == '.tflite':
            try:
                ...
                return None
        # Unknown extension: try generic load (safe catch)
        try:
            return tf.keras.models.load_model(path)
        except Exception as e:
            print(f"Generic load failed for '{path}': {e}")
            return None
    except Exception as e:
        print(f"Unexpected error while loading model '{path}': {e}")
        return None

# Primary attempt
model = try_load_model(MODEL_PATH)

# If not available, search common model files in project root
if model is None:
    print("Primary model failed; searching for other model files in project...")
    candidates = glob.glob("**/*.h5", recursive=True) + glob.glob("**/*.keras", recursive=True) + \
                 glob.glob("**/*.hdf5", recursive=True) + glob.glob("**/*.tflite", recursive=True)
    # keep unique, prefer first sensible candidate
    seen = []
    candidates = [c for c in candidates if not (c in seen or seen.append(c))]
    if candidates:
        print("Found candidate model files:")
        for c in candidates:
            print("  ", c)
        for c in candidates:
            m = try_load_model(c)
            if m is not None:
                ...
                print(f"Loaded model from fallback: {MODEL_PATH}")
                break
...
if not MODEL_AVAILABLE:
    print(
        ...
        "- If your model is a SavedModel directory, set MODEL_PATH to that directory.\n"
        "- If it's a .tflite file, it's supported (will use TFLite interpreter).\n"
        "- If it's a .h5 and you see HDF5 errors, the file may be corrupted or not a valid HDF5 file.\n"
        "App will continue running but predictions are disabled."
    )
...
class SignApp:
    ...
    def setup_window(self):
        ...
        self.root.title("🤟 ISL SMART COMMUNICATOR")
    ...
    def create_widgets(self):
        ...
        self.title_label = tk.Label(
            self.main_frame, 
            text="Indian Sign Languages to Text Translation", 
                        ...
                        if xs and ys:  # Ensure we have landmarks
                            ...
                            if crop.size > 0 and crop.shape[0] > 10 and crop.shape[1] > 10:
                                try:
                                    ...
                                    # Guarded prediction: only call predict when a model is loaded
                                    probs = None
                                    idx = -1
                                    conf = 0.0
                                    if MODEL_AVAILABLE and model is not None:
                                        ...
                                        # No model loaded - skip prediction
                                        probs = None
                                        idx = -1
                                        conf = 0.0
                                    
                                    # Display prediction and confidence on camera feed
                                    if conf >= 0.3 and probs is not None:  # Show predictions above 30% confidence
                                        pred_label = CLASS_NAMES[idx] if 0 <= idx < len(CLASS_NAMES) else str(idx)
                                    ...
                                    # Higher confidence threshold for insertion remains unchanged
                                    if conf >= 0.90 and probs is not None:
                                        ...
                                        should_insert = (label != self.last_label or 
                                                        self.frames_since_last_insert > 15)
